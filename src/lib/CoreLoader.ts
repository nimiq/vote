...
const coreBasePath = `https://cdn.jsdelivr.net/npm/@nimiq/core-web@v${CORE_VERSION}/`;
 ...
 */
export async function loadNimiqWithCryptography(mainnet: boolean = true): Promise<Nimiq> {
    nimiqCryptographyPromise = nimiqCryptographyPromise || (async () => {
        ...
        if (mainnet) Nimiq.GenesisConfig.main(); else Nimiq.GenesisConfig.test();
        return Nimiq;
    })();
